...
public interface AdminSubscriptionRepository extends JpaRepository<AdminSubscription, Long> {
    ...
    @Query("""
        update AdminSubscription s
        set s.status = case
            when s.active = true then com.pfe.promotionplatform.domain.model.AdminSubscriptionStatus.ACTIVE
            else com.pfe.promotionplatform.domain.model.AdminSubscriptionStatus.CANCELED
        end
